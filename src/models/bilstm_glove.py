...
Uses pre-trained GloVe word vectors in an Embedding layer with BiLSTM.
"""
...
def load_glove(path):
    """Load GloVe embeddings into a dict."""
    embeddings_index = {}
...
def main():
    ...
    print(f"  Loaded {len(embeddings_index):,} vectors, dim={embed_dim}")

    # Load and preprocess (keep stopwords for sequence)
    ...
    for f in ["confusion_matrix_bilstm_glove.png", "roc_curve_bilstm_glove.png"]:
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "bilstm_glove_history.png", dpi=300, bbox_inches="tight")
    plt.show()
    shutil.copy(FIGURES_DIR / "bilstm_glove_history.png", FIGURES_BILSTM / "bilstm_glove_history.png")

    # Save
    save_results(
        metrics,
        str(RESULTS_DIR / "results_bilstm_glove.json"),
        additional_info={"model": MODEL_NAME, "embedding": "GloVe", "embed_dim": embed_dim, "maxlen": MAXLEN},
    )
    model.save(str(RESULTS_DIR / "model_bilstm_glove.keras"))
    import joblib
